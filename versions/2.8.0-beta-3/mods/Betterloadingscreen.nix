...
Note: if a resourcepack adds a font, it will be used by BLS. [default: textures/font/ascii.png]";
          ...
          useMinecraft = lib.mkOption {
            type = lib.types.bool;
            default = true;
            description = "Whether or not to use minecraft's display to show the progress. This looks better, but there is a possibility of not being compatible, so if you do have any strange crash reports or compatibility issues, try setting this to false
