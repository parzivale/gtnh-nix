          ...
          type = lib.types.submodule {
            options = {
              alphaDecreaseStep = lib.mkOption {
                type = lib.types.str;
                default = "0.05";
                description = "Float from 0 to 1. The amount of alpha that is removed from the original image and added to the image that comes after. Also defined smoothnes of animation. Don't set this too low this time or you'll add time to your pack loading. Probably 0.01 still is too low. [range: 0.0 ~ 1.0, default: 0.01]";
              };
              ...
              threadSleepTime = lib.mkOption {
                type = lib.types.int;
                default = 20;
                description = "Time in milliseconds between each image change (smooth blend). The animation runs on the main thread (because OpenGL bruh momento), so setting this higher than default is not recommended (basically: if image transition running, your mods not loading) [range: 0 ~ 9000, default: 20]";
              };
            };
          };
        };
          ...
          type = lib.types.submodule {
            options = {
              connectExternally = lib.mkOption {
                type = lib.types.bool;
                default = false;
                description = "If this is true, it will conect to drone.io to get a changelog. Note: this repo is down. [default: false]";
              };
              font = lib.mkOption {
                type = lib.types.str;
                default = "textures/font/ascii.png";
                description = "What font texture to use? Special Cases: - If you use the Russian mod \"Client Fixer\" then change this to \"textures/font/ascii_fat.png\" Note: if a resourcepack adds a font, it will be used by CLS. [default: textures/font/ascii.png]";
              };
          ...
          type = lib.types.submodule {
            options = {
              ...
              titlePos = lib.mkOption {
                ...
                description = "Logo coordinates in image and position. the first four values indicate where the logo is located on the image (you could use a spritesheet), the four next ones tell where the image will be located on screen like this: [xLocation, yLocation, xWidth, yWidth, xLocation, yLocation, xWidth, yWidth] The same is used for other images, except the background, which is fullscreen. Please ALWAYS provide an image, a transparent one if you want even. CLS provides 'transparent.png' If you really insist ping me and I'll look if I can add an image or so. jackowski626#0522 [default: [0, 0, 256, 256, 0, 50, 187, 145]]";
              };
            };
          };
        };
          ...
          type = lib.types.submodule {
            options = {
              baseTipsTextPos = lib.mkOption {
                ...
                description = "Base text position. Can be TOP_CENTER, TOP_RIGHT, CENTER_LEFT, CENTER, CENTER_RIGHT, BOTTOM_LEFT, BOTTOM_CENTER or BOTTOM_RIGHT. Note: Other elements use CENTER, if you really need, ask to implement this base poition option for any other element. [default: BOTTOM_CENTER]";
              };
