          ...
          type = lib.types.submodule {
            options = {
              ...
              newBoatEntityBlacklist = lib.mkOption {
                ...
                default = [ ];
                description = "What entities shouldn't be able to sit in the boat? You can either provide an entity ID (modid.entityid, for vanilla entities type just entity ID), or search for a string in the classpath (classpath:stringtofind). Separate entries in the list by a new line. Note that players can always sit even if blacklisted, and some entities, like horses, water mobs or nonliving entities, will never be allowed to sit in boats. It's a little hard to explain, a more detailed explanation and list of examples can be found here: https://pastebin.com/XNZ7VWKh";
              };
          ...
          type = lib.types.submodule {
            options = {
              ...
              fossilBlockID = lib.mkOption {
                ...
                description = "Use a namespaced ID, + optionally meta (max 3) to choose the block that makes up fossils. The max meta is 3 because the rotations will change the meta. North/South is the meta + 4 and East/West is + 8. Netherlicious bone block is \"netherlicious:BoneBlock\" and UpToDate bone block is \"uptodate:bone_block\". If the chosen block does not exist then fossils will not generate. If Netherlicious is installed, its bone block will be used if this is set to \"etfutrum:bone_block\" and Et Futurum Requiem bone blocks are disabled. [default: etfuturum:bone]";
              };
