          ...
          type = lib.types.submodule {
            options = {
              ...
              "clear.history.after.food.eaten.threshold.reached" = lib.mkOption {
                type = lib.types.bool;
                default = false;
                description = "If true, a player's food history will be empty once they pass the new.player.food.eaten.threshold If false, any food eaten before the threshold is passed will also count after it is passed";
              };
              "food.containers.chance.to.drop.food" = lib.mkOption {
                type = lib.types.float;
                default = 0.25;
                description = "The chance for food to drop from an open food container when the player jumps Temporarily disabled while a better implementation is written (this config option will do nothing)";
              };
              ...
              "give.food.journal.on.dimishing.returns.start" = lib.mkOption {
                type = lib.types.bool;
                default = false;
                description = "If true, a food journal will be given to each player once diminishing returns start for them Not given if a player was given a food journal by give.food.journal.as.starting.item";
              };
              ...
              "use.time.for.food.history.length" = lib.mkOption {
                type = lib.types.bool;
                default = false;
                description = "If true, food.history.length will use time (in Minecraft days) instead of number of foods eaten for its maximum length For example, a food.history.length length of 12 will store all foods eaten in the last 12 Minecraft days. Note: On servers, time only advances for each player while they are logged in unless use.time.progress.time.while.logged.off is set to true Also note: use.hunger.restored.for.food.history.length must be false for this config option to take effect";
              };
              "use.time.progress.time.while.logged.off" = lib.mkOption {
                type = lib.types.bool;
                default = false;
                description = "If true, food history time will still progress for each player while that player is logged out. NOTE: use.time.for.food.history.length must be true for this to have any affect";
              };
            };
          };
        };
