      ...
      general = lib.mkOption {
        ...
        type = lib.types.submodule {
          ...
          maxDams = lib.mkOption {
            type = lib.types.int;
            default = 16;
            description = "[SERVER] How many dams should the game support. At least as many as the server you want to connect to. Each dam will receive it's own water block and it will also have a minuscule performance impact. Keep it only as long as you need. You can always just rise, but not shorten the value.";
          };
          ...
          minimalWaterUpdateInterval = lib.mkOption {
            type = lib.types.int;
            default = 1000;
            description = "[SERVER] Minimum delay in milliseconds beween update packets from the server to ALL clients.";
          };
