      ...
      settings = lib.mkOption {
        ...
        type = lib.types.submodule {
          ...
          LeaveDecayFuzz = lib.mkOption {
            type = lib.types.int;
            default = 5;
            description = "A random number from 0-thisconfigoption will be added to the decay speed for every leaf.
...
thing look more natural";
          ...
          LeaveDecaySpeed = lib.mkOption {
            type = lib.types.int;
            default = 7;
            description = "The amount of ticks every leaf needs to decay (Lower is faster)";
          };
          ...
          SpectreDimensionID = lib.mkOption {
            type = lib.types.int;
            default = 2;
            description = "The Dimension ID of the spectre World. On first run and when you set this setting to -1
RandomThings will try to find a dimensionID itself";
          ...
          SpiritChanceSword = lib.mkOption {
            type = lib.types.float;
            default = 0.2;
            description = "The chance of a spirit spawning when you have a spirit binder in your inventory and kill
      ...
      vanillachanges = lib.mkOption {
        ...
        type = lib.types.submodule {
          ...
          "Hardcore Darkness" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "The minimum light will be removed so if there's no light source it's actually going to be
completely black";
          };
          ...
          ThrowableMotion = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "When you throw something or shoot an arrow the motion of the player will be added to the
motion of the projectile";
          };
          fixedBackground = lib.mkOption {
            type = lib.types.str;
            default = "";
            description = "If this is not empty the options background will not be random but the one specified here.
