      ...
      server = lib.mkOption {
        ...
        type = lib.types.submodule {
          ...
          "affect.food.hunger.values" = lib.mkOption {
            type = lib.types.bool;
            default = true;
            description = "If true, foods' hunger value will be multiplied by the current nutritional value
Setting this to false and affect.food.saturation.modifiers to true will make diminishing returns affect saturation only";
          ...
          "affect.food.saturation.modifiers" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, foods' saturation modifier will be multiplied by the current nutritional value
...
Setting this to true and affect.food.hunger.values to false will make diminishing returns affect saturation only";
          ...
          "affect.negative.food.hunger.values" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, foods with negative hunger values will be made more negative as nutritional value decreases
NOTE: affect.food.hunger.values must be true for this to have any affect";
          ...
          "affect.negative.food.saturation.modifiers" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, foods with negative saturation modifiers will be made more negative as nutritional value decreases
NOTE: affect.food.saturation.modifiers must be true for this to have any affect";
          ...
          "clear.history.after.food.eaten.threshold.reached" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, a player's food history will be empty once they pass the new.player.food.eaten.threshold
If false, any food eaten before the threshold is passed will also count after it is passed";
          ...
          "food.containers.chance.to.drop.food" = lib.mkOption {
            ...
            default = 0.25;
            description = "The chance for food to drop from an open food container when the player jumps
Temporarily disabled while a better implementation is written (this config option will do nothing)";
          ...
          "food.eating.duration.max" = lib.mkOption {
            type = lib.types.int;
            default = 0;
            description = "The maximum time it takes to eat a food after being modified by food.eating.speed.modifier
...
Note: If this is set to 0 and food.eating.speed.modifier is > 0, a food with 0% nutrtional value will take nearly infinite time to eat";
          ...
          "food.eating.speed.modifier" = lib.mkOption {
            type = lib.types.float;
            default = 1.0;
            description = "If set to greater than zero, food eating speed will be affected by nutritional value
...
Eating duration is calcualted using the formula (eating_duration / (nutritional_value^eating_speed_modifier))";
          ...
          "food.hunger.rounding.mode" = lib.mkOption {
            type = lib.types.str;
            default = "floor";
            description = "Rounding mode used on the hunger value of foods
Valid options: 'round', 'floor', 'ceiling'";
          };
          "food.modifier.formula" = lib.mkOption {
            type = lib.types.str;
            default = "IF(count>=3 && distinct_food_groups_eaten<=5,(1 - (count - 3)/4),1)";
            description = "Uses the EvalEx expression parser
...
";
          ...
          "give.food.journal.on.dimishing.returns.start" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, a food journal will be given to each player once diminishing returns start for them
Not given if a player was given a food journal by give.food.journal.as.starting.item";
          ...
          "use.hunger.restored.for.food.history.length" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, food.history.length will use amount of hunger restored instead of number of foods eaten for its maximum length
...
NOTE: food.history.length uses hunger units, where 1 hunger unit = 1/2 hunger bar";
          ...
          "use.time.for.food.history.length" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, food.history.length will use time (in Minecraft days) instead of number of foods eaten for its maximum length
...
Also note: use.hunger.restored.for.food.history.length must be false for this config option to take effect";
          ...
          "use.time.progress.time.while.logged.off" = lib.mkOption {
            type = lib.types.bool;
            default = false;
            description = "If true, food history time will still progress for each player while that player is logged out.
